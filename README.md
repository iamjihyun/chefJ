# chefJ
##기획의도
* 셰프제이는 레시피 공유 및 식자재 구매 원스톱 플랫폼 웹서비스로 제작한 프로젝트입니다. 1인 가구가 증가하며 바쁘더라도 모두 함께 건강한 식단을 섭취하자는 취지로 식자재 장바구니에서 구매까지, 레시피 등록, 조회, 댓글기능, 일반게시판 조회 및 글쓰기, 삭제, 관리자페이지, 고객센터 기능 등 전반적인 기능을 제공.
* 개발기간 : 1.5개월(설계, 기획, 개발, 테스트)
* 제작인원 : 4명
...
* 로그인/회원가입
  - 유효성검사, 이메일인증, 도로명 주소, 아이디 중복검사(ajax)
